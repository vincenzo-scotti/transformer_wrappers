    ...
    for exp_config in configs['exp']['params']:
        ...
        for task in tasks:
            ...
            results_dump_path = os.path.join(
                os.path.join(configs['current_experiment_dir_path'],  f'results_{task}_{exp_config_hash}.json')
            )
            #
            if not (os.path.exists(results_dump_path) and os.path.exists(configs_dump_path)) or configs['exp'].get('overwrite', False):
                # Set attn len
                ...
                if results is not None:
                    ...
                    logging.info(f"Results on `{task}` serialised")
                else:
                    logging.error(f"Null results on `{task}` for configuration: `{exp_config}`")
    # Close script info
